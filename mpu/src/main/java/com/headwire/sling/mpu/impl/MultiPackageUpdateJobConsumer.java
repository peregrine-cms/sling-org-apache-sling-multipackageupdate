...
public final class MultiPackageUpdateJobConsumer implements JobConsumer {
	...
	public JobResult process(final Job job) {
		...
		logger.info("Process Job, Endpoint: '{}'", endpoint != null ? endpoint.getServerUrl() : "NO ENDPOINT");
		...
		try {
			...
			if (processPerformerListener.setProcessPerformer(endpoint.getName(), performer)) {
				logger.info("Process Job, Run Performer: '{}'", performer);
				performer.run();
			}
			...
			endpoint.log("Failed to Start Job: " + e.getLocalizedMessage());
